...
$(document).ready(function(){
	...
	//add event listener
	//only works for things in the DOM on page load
	// $('li').click(function(){ //here we're using an anonymous function
	// 	$(this).css('text-decoration', 'line-through')
	// });

	
	$('.add').click(addItem); //here we're using a named function

	//delegated click event - works on everything no matter when it was created
	$('.todoList').on('click', 'li', function(element){ //"function(element)" is callback function
		$(element.target).css('text-decoration', 'line-through');
		//element.target looks at the item you directly clicked on
	});
